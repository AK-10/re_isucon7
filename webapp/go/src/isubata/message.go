...
func queryMessagesWithJsonify(chanID, lastID int64) ([]map[string]interface{}, error) {
	...
	query := `SELECT u.name, u.display_name, u.avatar_icon, m.id, m.create_at, m.content FROM user u INNER JOIN message m ON u.id = m.user_id WHERE m.id > ? AND m.channel_id = ? ORDER BY m.id DESC LIMIT 100`
